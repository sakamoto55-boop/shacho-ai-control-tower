...
社長がcurlを使わずに手入力できる、最小限の1枚ページです。管理画面ではないため、保存済み一覧やナビゲーションはありません。本文を貼り付けて「AI分析する（保存しない）」または「AI分析して保存する」を押すと、分析結果・タスク・返信下書き・会社運営チームレビューがその場に表示されます。

```
http://localhost:8787/dev/console
```
