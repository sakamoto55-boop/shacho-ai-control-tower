...
| AI相談チャット | 6モード（AI秘書/AI経営/AI現場/AI事務/AI営業/AI福祉）のタブ切替チャット |
...
**Phase 2では外部サービスへの接続は一切行っていません。**

- Gmail、LINE WORKS、freee、Google Drive、Googleカレンダー、Googleスプレッドシート、TKC等の既存データへの読み取り・書き込みはゼロです。
- 表示されているすべての数値・メッセージ・タスク・グラフは `client/src/data/mockData.ts` 内の仮データです。
...
| フェーズ | 機能 | 対応時期 |
|---------|------|---------|
| Phase 3-1 | Gmail読み取り（受信メールの取得） | 次回 |
| Phase 3-2 | Googleカレンダー読み取り | 次回 |
| Phase 3-3 | Google Drive検索 | 次回 |
| Phase 3-4 | Googleスプレッドシート読み取り | 次回 |
| Phase 3-5 | LINE WORKS通知受信 | 未定 |
| Phase 3-6 | Claude APIリアルタイム接続 | 未定 |
| Phase 4-1 | Gmail下書き作成（送信なし） | 将来 |
| Phase 4-2 | Google Drive保存 | 将来 |
