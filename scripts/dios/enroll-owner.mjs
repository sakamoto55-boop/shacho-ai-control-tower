...
try{await db.query("INSERT INTO dios_members(tenant_id,email,principal) VALUES($1,$2,$3::jsonb) ON CONFLICT DO NOTHING",[db.tenantId,email,JSON.stringify({role:'PRESIDENT',companyIds:companies,label:email})]);console.log('Owner enrolment completed; existing accounts were not altered');}
